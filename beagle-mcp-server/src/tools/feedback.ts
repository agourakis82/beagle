...
const TagRunSchema = z.object({
  ...
  accepted: z.boolean().describe('Whether the run output was accepted (good quality)'),
  rating0_10: z.number().int().min(0).max(10).optional().describe('Rating from 0-10'),
  notes: z.string().optional().describe('Additional notes'),
});

const TagExperimentRunSchema = z.object({
  experiment_id: z.string().describe('Experiment ID (e.g., "triad_vs_ensemble", "hrv_aware_vs_blind")'),
  run_id: z.string().describe('Run ID to tag'),
  condition: z.string().describe('Experimental condition (e.g., "triad", "ensemble", "hrv_aware")'),
  notes: z.string().optional().describe('Additional notes'),
});
...
- Quality analysis`,
      inputSchema: {
        ...
        properties: {
          ...
          accepted: {
            type: 'boolean',
            description: 'Whether the run output was accepted (good quality)',
          },
          rating0_10: {
            type: 'number',
      ...
      handler: async (args: unknown) => {
        const { run_id, accepted, rating0_10, notes } = TagRunSchema.parse(args);
        
        const result = await client.tagRun(run_id, accepted, rating0_10, notes);
...
The tagged runs can be analyzed later to compare conditions.`,
      inputSchema: {
        ...
        properties: {
          experiment_id: {
            type: 'string',
            description: 'Experiment ID (e.g., "triad_vs_ensemble", "hrv_aware_vs_blind")',
          },
          run_id: {
            type: 'string',
            description: 'Run ID to tag',
          ...
          condition: {
            type: 'string',
            description: 'Experimental condition (e.g., "triad", "ensemble", "hrv_aware")',
          },
          notes: {
            type: 'string',
            description: 'Additional notes (optional)',
      ...
      handler: async (args: unknown) => {
        const { experiment_id, run_id, condition, notes } = TagExperimentRunSchema.parse(args);
        
        const result = await client.tagExperimentRun(experiment_id, run_id, condition, notes);
