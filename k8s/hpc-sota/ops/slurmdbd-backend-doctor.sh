...
elif [[ "${storage_lines}" == *"StorageType=accounting_storage/mysql"* && "${storage_lines}" == *"StorageHost=slurm-pilot-mariadb"* ]]; then
  pass "slurmdbd is pointed at the externalized MariaDB candidate via compatibility service slurm-pilot-mariadb:3306"
