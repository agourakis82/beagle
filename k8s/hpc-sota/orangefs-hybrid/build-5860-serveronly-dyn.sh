...
if [[ -n "${SSHPASS:?defina SSHPASS no ambiente (nunca no script); prefira chave SSH}" ]]; then
  SSH_BASE=(sshpass -p "${SSHPASS:?defina SSHPASS no ambiente (nunca no script); prefira chave SSH}" ssh -o StrictHostKeyChecking=no)
fi
