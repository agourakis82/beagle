...
JOB_NAME="orangefs-text-integrity-probe-${TARGET_NODE%%-*}"
TMP_MANIFEST="$(mktemp)"
cleanup() {
  rm -f "$TMP_MANIFEST"
}
...
python3 - <<'PY' "$BASE/k8s/job-orangefs-text-integrity-probe.yaml" "$TMP_MANIFEST" "$JOB_NAME" "$TARGET_NODE"
...
echo "[2/4] clear previous text-integrity job if present"
remote_k "-n $NAMESPACE delete job $JOB_NAME --ignore-not-found=true"
sleep 2

echo "[3/4] apply OrangeFS text-integrity probe on $TARGET_NODE"
