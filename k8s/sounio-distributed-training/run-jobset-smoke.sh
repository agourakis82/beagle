...
manifest="${root}/jobset-gpu-ddp-smoke.yaml"
jobset_name="sounio-gpu-ddp-smoke"
namespace="beagle"
...
old_pods="$(k -n "${namespace}" get pods -l jobset.sigs.k8s.io/jobset-name="${jobset_name}" --no-headers 2>/dev/null | wc -l | tr -d ' ')"
if [[ "${old_pods}" != "0" ]]; then
  k -n "${namespace}" delete pods -l jobset.sigs.k8s.io/jobset-name="${jobset_name}" --force --grace-period=0 >/dev/null 2>&1 || true
fi
k apply -f "${manifest}"
