...
use beagle_fractal::{get_root, init_fractal_root};
...
async fn main() -> Result<()> {
    ...
    info!(
        "✅ Fractal root inicializado - ID: {}, Depth: {}",
        root.id, root.depth
    );
    assert!(root.id > 0);
    ...
    info!(
        "🔄 Testando query_beagle() com contexto pequeno ({} tokens)...",
        small_context
    );
    ...
    info!(
        "🔄 Testando query_beagle() com contexto grande ({} tokens)...",
        large_context
    );
    ...
    if !large_response.is_empty() && !large_response.starts_with("ERRO") {
        info!(
            "✅ query_beagle() respondeu com contexto grande: {} chars",
            large_response.len()
        );
    ...
    info!(
        "🔄 Testando alinhamento cosmológico com {} hipóteses...",
        initial_count
    );
    ...
    match cosmo.align_with_universe(&mut test_set).await {
        Ok(()) => {
            ...
            info!(
                "✅ Alinhamento cosmológico concluído - {} → {} hipóteses",
    ...
    match void
        .navigate_void(1, "teste e2e completo do sistema beagle")
        .await
    {
        Ok(result) => {
            info!(
                "✅ Navegação no void concluída - {} ciclos, {} insights",
    ...
    if set.hypotheses.is_empty() {
        set.add(
            "Hipótese de teste E2E 1: Emergência quântica em sistemas biológicos".to_string(),
            None,
        );
        set.add(
            "Hipótese de teste E2E 2: Causalidade reversa em processos celulares".to_string(),
            None,
        );
