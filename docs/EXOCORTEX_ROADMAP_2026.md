...
- **Continuous RAG update (Rust)**: `crates/beagle-rag-update/`
  ...
  - `darwin-brief` (topic briefs from current corpus + citations)
- **Ingestion ledger**: optional Postgres ledger (`DARWIN_LEDGER_DATABASE_URL`) to skip unchanged docs and track statuses.
- **Eval harness**: `darwin-eval` (golden queries → pass/fail + MRR summary).
...
- **Auto-update via systemd**: `scripts/systemd/` timers for incremental indexing, research harvesting, and a nightly harvest→eval pipeline.
...
4. **Cost-aware multi-model**: default routing favors cheap/high-rate providers (MiniMax/Z.ai → Grok → DeepSeek) with hard quotas.
...
- Nightly pipeline: systemd wrapper runs harvest + brief + eval + webhook notifications.
- Implemented: “what changed since last week” briefs via `darwin-brief --delta-days 7` (brief state history).
- Remaining: richer citation validity checks.
...
- Implemented: `darwin-eval` binary that:
  - runs “golden queries” → asserts expected `doc_id` appears in top-k
- Implemented: trend history (`~/.darwin_eval_history.jsonl`) and drift gates (`--max-mrr-drop`, `--min-mrr`).
- Remaining: richer citation validity checks.

### F) Integrations (P2)

- Implemented: MCP tooling exposes “trigger index / trigger harvest / status” via the existing MCP server (`beagle-mcp-server/` + core server Darwin jobs API).
- Implemented: one-call nightly ResearchOps MCP tools:
  - `beagle_darwin_nightly_workflow_minimax` (MiniMax → Grok → DeepSeek)
  - `beagle_darwin_nightly_workflow_zai` (Z.ai → Grok → DeepSeek)
- Implemented: portable systemd installer helper: `scripts/systemd/install-darwin-timers.sh`
...
## Provider policy (Z.ai → Grok → DeepSeek)

- Set:
  - `BEAGLE_ROUTING_POLICY=zai-grok-deepseek`
  - `ZAI_API_KEY=...` (rotate if ever pasted into chat/logs)
  - `ZAI_API_BASE=https://open.bigmodel.cn/api/paas/v4` (or your OpenAI-like base)
  - `ZAI_MODEL=GLM-4.7`
  - `DEEPSEEK_API_KEY=...` (only for math tier)

## Build note (Ubuntu 26 / Python 3.13)

If `cargo` fails with PyO3 “Python 3.13 newer than supported”, the repo already enables:
`PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1` via `.cargo/config.toml`.
