...
### Added
- MCP: legacy SSE transport endpoints (`/sse`, `/message`) for claude.ai connector compatibility.
- Docs: public exposure runbook (`docs/MCP_EXTERNAL_ACCESS.md`) + Cloudflare Tunnel systemd unit template (`scripts/systemd/cloudflared-beagle.service`).

### Changed
- MCP: HTTP endpoint-level bearer auth option for `/mcp`, `/sse`, and `/message` (keeps `/health` public).

## [0.27.1] - 2026-01-22

### Fixed
- systemd: `beagle-core.service` and `beagle-mcp.service` no longer fail due to systemd environment expansion inside `bash -lc` (use `WorkingDirectory` + direct `ExecStart`)
