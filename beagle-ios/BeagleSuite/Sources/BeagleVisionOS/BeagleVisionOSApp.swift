            ...
            ScrollView {
                VStack(spacing: 10) {
                    ...
                    Lane(title: "Cluster", truth: store.clusterTruth.mode) {
                        Text("3 nodes healthy")
                            .font(BeagleTheme.dataFont(size: 12))
                            .foregroundStyle(BeagleTheme.textData)
                    }
                    Lane(title: "Research", truth: store.research.mode) {
                        Text("latest: brain-ossm-abide")
                            .font(BeagleTheme.dataFont(size: 12))
                            .foregroundStyle(BeagleTheme.textData)
                    }
...
struct ClusterVolumeView: View {
    ...
    var body: some View {
        RealityView { content in
            // Create 3 node entities positioned in space
            let r770 = makeNode(name: "r770", color: BeagleTheme.truthObserved, position: SIMD3<Float>(-0.4, 0.3, 0))
            let r740 = makeNode(name: "r740", color: BeagleTheme.truthObserved, position: SIMD3<Float>(0.4, 0.3, 0))
            let t560 = makeNode(name: "t560", color: BeagleTheme.truthDeclared, position: SIMD3<Float>(0, -0.3, 0))
            ...
            content.add(makeEdge(from: SIMD3<Float>(-0.4, 0.3, 0), to: SIMD3<Float>(0.4, 0.3, 0)))
            content.add(makeEdge(from: SIMD3<Float>(-0.4, 0.3, 0), to: SIMD3<Float>(0, -0.3, 0)))
            content.add(makeEdge(from: SIMD3<Float>(0.4, 0.3, 0), to: SIMD3<Float>(0, -0.3, 0)))
        }
        .installGestures()
    }
