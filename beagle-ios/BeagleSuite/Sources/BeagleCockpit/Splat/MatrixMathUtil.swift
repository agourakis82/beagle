...
// World→view (camera looks down -Z in view space) — matches matrix_perspective_right_hand.
func matrix_look_at_right_hand(eye: SIMD3<Float>, center: SIMD3<Float>, up: SIMD3<Float>) -> matrix_float4x4 {
    let f = normalize(center - eye)      // forward, toward target
    let s = normalize(cross(f, up))      // right
    let u = cross(s, f)                  // recomputed up
    return matrix_float4x4.init(columns:(vector_float4( s.x,  u.x, -f.x, 0),
                                         vector_float4( s.y,  u.y, -f.y, 0),
                                         vector_float4( s.z,  u.z, -f.z, 0),
                                         vector_float4(-dot(s, eye), -dot(u, eye), dot(f, eye), 1)))
}
