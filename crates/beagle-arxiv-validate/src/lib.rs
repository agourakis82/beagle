...
impl ArxivValidator {
    ...
    pub fn validate(&self, tex_path: &str) -> Result<Vec<String>> {
        ...
        let content = fs::read_to_string(tex_path).context("Falha ao ler arquivo LaTeX")?;
        ...
        if !has_bibliography {
            errors.push(
                "⚠️  Nenhuma referência encontrada (arXiv prefere papers com referências)"
                    .to_string(),
            );
        }
        ...
        if word_count < 2000 {
            errors.push(format!(
                "❌ Paper muito curto ({} palavras) — arXiv prefere >2000",
                word_count
            ));
        }
        ...
        let has_sections = content.contains("\\section{") || content.contains("\\subsection{");
    ...
    pub fn validate_markdown(&self, markdown_path: &str) -> Result<Vec<String>> {
        ...
        let mut errors = Vec::new();
        let content = fs::read_to_string(markdown_path).context("Falha ao ler markdown")?;
        ...
        if section_count < 3 {
            errors.push(format!(
                "❌ Paper tem poucas seções ({}) — arXiv prefere pelo menos 3",
                section_count
            ));
        }
