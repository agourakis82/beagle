...
use beagle_rag_update::knowledge::{
    index_document, DocType, DocumentMeta, KnowledgeConfig, MetadataEnricher,
};
...
async fn main() -> Result<()> {
    ...
    let metadata_enricher = if args.enrich_metadata {
        Some(Arc::new(MetadataEnricher::from_env()))
    } else {
        None
    };
