...
struct Args {
    ...
    /// Optional repo targets file(s) (YAML/TOML/JSON).
    /// Example:
    /// repos:
    ///   - path: /home/demetrios/beagle
    ///   - url: https://github.com/agourakis82/beagle.git
    #[arg(long, env = "DARWIN_REPOS_FILE", value_delimiter = ',', num_args = 1..)]
    repos_file: Vec<String>,

    /// Fetch public repos from a GitHub user (repeatable; comma-separated via env).
    #[arg(long, env = "DARWIN_GITHUB_USER", value_delimiter = ',', num_args = 1..)]
    github_user: Vec<String>,

    /// Fetch public repos from a GitHub org (repeatable; comma-separated via env).
    #[arg(long, env = "DARWIN_GITHUB_ORG", value_delimiter = ',', num_args = 1..)]
    github_org: Vec<String>,

    /// Fetch repos from the authenticated GitHub user (may include private repos).
    ///
    /// Requires a token via DARWIN_GITHUB_TOKEN/GITHUB_TOKEN/GH_TOKEN.
    #[arg(long, env = "DARWIN_GITHUB_ME")]
    github_me: bool,

    /// GitHub API base URL (for GitHub Enterprise).
    #[arg(long, env = "DARWIN_GITHUB_API_BASE", default_value = "https://api.github.com")]
    github_api_base: String,

    /// Optional GitHub token (prefer env: DARWIN_GITHUB_TOKEN/GITHUB_TOKEN/GH_TOKEN).
    #[arg(long, env = "DARWIN_GITHUB_TOKEN")]
    github_token: Option<String>,

    /// Include fork repos when listing via GitHub API.
    #[arg(long, env = "DARWIN_GITHUB_INCLUDE_FORKS")]
    github_include_forks: bool,

    /// Prefer SSH clone URLs for GitHub-discovered repos (useful for private repos with SSH keys).
    #[arg(long, env = "DARWIN_GITHUB_CLONE_SSH")]
...
fn dedup_repo_targets(repos: &mut Vec<RepoTarget>) {
    let mut seen: HashSet<(String, String)> = HashSet::new();
    repos.retain(|r| {
        seen.insert((r.name.clone(), r.path.to_string_lossy().to_string()))
    });
}

fn pick_github_repo_url(repo: &beagle_rag_update::github::GitHubRepo, prefer_ssh: bool) -> Option<String> {
    if prefer_ssh {
        repo.ssh_url.clone().or_else(|| repo.clone_url.clone())
    } else {
        repo.clone_url.clone().or_else(|| repo.ssh_url.clone())
    }
}
...
async fn main() -> Result<()> {
    ...
    for f in &args.repos_file {
        let path = expand_tilde(f);
        let mut from_file = load_repo_targets_file(&path, &repos_dir)?;
        repos.append(&mut from_file);
    }
    ...
    if !args.github_user.is_empty() || !args.github_org.is_empty() || args.github_me {
        let token = args.github_token.clone().or_else(GitHubClient::token_from_env);
        let gh = GitHubClient::new(args.github_api_base.clone(), token)?;

        for user in &args.github_user {
            let repos_json = gh
                .list_user_repos(user, args.github_include_forks)
                .await?;
            for r in repos_json {
                let Some(url) = pick_github_repo_url(&r, args.github_clone_ssh) else { continue };
                let name = r.name;
                let path = repos_dir.join(&name);
                repos.push(RepoTarget {
                    name,
        ...
        for org in &args.github_org {
            let repos_json = gh
                .list_org_repos(org, args.github_include_forks)
                .await?;
            for r in repos_json {
                let Some(url) = pick_github_repo_url(&r, args.github_clone_ssh) else { continue };
                let name = r.name;
        ...
        if args.github_me {
            let repos_json = gh
                .list_authenticated_user_repos(args.github_include_forks)
                .await?;
            for r in repos_json {
                let Some(url) = pick_github_repo_url(&r, args.github_clone_ssh) else { continue };
                let name = r.name;
    ...
    dedup_repo_targets(&mut repos);

    if repos.is_empty() {
        anyhow::bail!(
            "provide at least one repo source: --repo-path, --repo-url, --repos-file, --github-user, --github-org, or --github-me"
        );
    }
