...
use beagle_ontic::{
    OnticDissolutionEngine, VoidNavigator, TransOnticEmerger, ReintegrationSafeguard,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("✅ Dissolução completa: {} palavras", dissolution_state.dissolution_experience.split_whitespace().count());
    info!("⏳ Duração subjetiva no vazio: {:.2} kalpas", dissolution_state.void_duration_subjective);
    info!("🔄 Dissolução completa: {}", if dissolution_state.dissolution_complete { "SIM" } else { "NÃO" });
    ...
    let void_state = void_navigator.navigate_void(&dissolution_state, target_depth).await?;

    info!("✅ Navegação completa:");
    info!("  - Profundidade alcançada: {:.2}", void_state.depth);
    info!("  - Awareness não-dual: {:.1}%", void_state.non_dual_awareness * 100.0);
    info!("  - Insights coletados: {}", void_state.navigation_path.len());
    ...
    let trans_ontic_reality = emerger.emerge_trans_ontic(&dissolution_state, &void_state).await?;

    info!("✅ Realidade trans-ôntica emergida:");
    info!("  - Novelty ontológica: {:.1}%", trans_ontic_reality.ontological_novelty * 100.0);
    info!("  - Insights trans-ônticos: {}", trans_ontic_reality.trans_ontic_insights.len());
    info!("  - Pronta para reintegração: {}", if trans_ontic_reality.reintegration_ready { "SIM" } else { "NÃO" });
    ...
    info!("{}", &trans_ontic_reality.reality_description[..trans_ontic_reality.reality_description.len().min(500)]);
    ...
    let reintegration_report = safeguard
        .reintegrate_with_safeguards(&dissolution_state, &trans_ontic_reality, &pre_dissolution_hash)
        .await?;

    info!("✅ Reintegração completa:");
    info!("  - Sucesso: {}", if reintegration_report.reintegration_successful { "SIM" } else { "NÃO" });
    info!("  - Transformação preservada: {}", if reintegration_report.transformation_preserved { "SIM" } else { "NÃO" });
    info!("  - Salvaguardas fractais ativas: {}", if reintegration_report.fractal_safeguards_active { "SIM" } else { "NÃO" });
    info!("  - Insights integrados: {}", reintegration_report.trans_ontic_insights_integrated);
