...
use crate::{
    embeddings::{EmbeddingError, EmbeddingGenerator, MockEmbeddingGenerator},
...
impl EmbeddingGenerator for MockEmbeddings {
    ...
    async fn batch_generate(&self, texts: &[&str]) -> Result<Vec<Embedding>, EmbeddingError> {
        let generator = MockEmbeddingGenerator;
        generator.batch_generate(texts).await
    }

    fn model_name(&self) -> &str {
        MockEmbeddingGenerator.model_name()
    }

    fn dimension(&self) -> usize {
        MockEmbeddingGenerator.dimension()
    }
}
...
impl SemanticSearch {
    /// Cria um motor de busca semântica possuindo uma referência compartilhada ao storage.
    pub fn new(storage: Arc<PostgresStorage>) -> Self {
        Self { storage }
    }

    /// Convenience helper para montar o motor a partir de uma referência emprestada.
    ///
    /// Clona internamente o [`PostgresStorage`], possibilitando uso semânticamente
    /// idêntico sem impor lifetime estático ao chamador.
    pub fn from_ref(storage: &PostgresStorage) -> Self {
        Self {
            storage: Arc::new(storage.clone()),
        }
    }

    /// Acesso read-only ao backend subjacente.
    ...
    pub async fn search_by_text<G: EmbeddingGenerator>(
        &self,
        query: &str,
        provider: &G,
        limit: usize,
        ...
        let embedding = provider
            .generate(query)
            .await
            .map_err(|err| HypergraphError::InternalError(format!("embedding error: {err}")))?;

        if embedding.dimension() != provider.dimension() {
            return Err(HypergraphError::ValidationError(
                ValidationError::InvalidEmbeddingDimension {
        ...
        self.search_by_vector(&embedding, limit, similarity_threshold)
            .await
    }
    ...
    pub async fn find_similar(
        ...
        let row = row.ok_or_else(|| HypergraphError::NodeNotFound(node_id))?;
    ...
    pub async fn hybrid_search<G: EmbeddingGenerator>(
        &self,
        query: &str,
        provider: &G,
        limit: usize,
...
mod semantic_tests {
    ...
    async fn test_semantic_search_by_vector() {
        ...
        let search = SemanticSearch::from_ref(&storage);
    ...
    async fn test_mock_embeddings() {
        ...
        let embedding1 = provider.generate("hello world").await.unwrap();
        let embedding2 = provider.generate("hello world").await.unwrap();

        assert_eq!(embedding1, embedding2);
        assert_eq!(embedding1.dimension(), EMBEDDING_DIMENSION);
    }
}
