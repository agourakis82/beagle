...
impl WhisperNeuralEngine {
    ...
    pub async fn transcribe(&self, audio_path: &str) -> Result<String> {
        ...
        if !Path::new(audio_path).exists() {
            return Err(anyhow::anyhow!("Arquivo de áudio não encontrado: {}", audio_path));
        }
        
        info!("🎤 Transcrevendo áudio local com Neural Engine: {}", audio_path);
        ...
        if !output.status.success() {
            ...
            // Lê arquivo de saída
            let output_file = format!("{}.txt", audio_path.trim_end_matches(Path::new(audio_path).extension().and_then(|s| s.to_str()).unwrap_or("")));
            let transcription = std::fs::read_to_string(&output_file)
                .context("Falha ao ler arquivo de transcrição")?;
            
            info!("✅ Transcrição Neural Engine (Metal/CPU): {} chars", transcription.len());
        ...
        info!("✅ Transcrição Neural Engine (CoreML): {} chars, < 200ms", transcription.len());
    ...
    pub async fn transcribe_stream(&self, audio_stream: &[u8]) -> Result<String> {
        ...
        std::fs::write(temp_path, audio_stream)
            .context("Falha ao salvar stream temporário")?;
