...
impl WeaknessAnalyzer {
    ...
    pub async fn analyze_failures(&self, monitor: &PerformanceMonitor) -> Result<Vec<FailurePattern>> {
        ...
        let failure_summary = failures.iter()
            .map(|f| format!("Query: {} | Domain: {} | Error: {} | Quality: {:.2}", 
        ...
        let patterns: Vec<PatternData> = serde_json::from_str(&json_content)
            .unwrap_or_else(|_| vec![]);
        
        let failure_patterns: Vec<FailurePattern> = patterns.into_iter()
    ...
    pub async fn identify_missing_capabilities(&self, patterns: &[FailurePattern]) -> Result<Vec<String>> {
        let patterns_summary = patterns.iter()
