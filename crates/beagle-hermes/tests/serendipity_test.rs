...
async fn test_swanson_abc_discovery() {
    let engine = SerendipityEngine::new("bolt://localhost:7687", "neo4j", "hermespassword", 0.7)
        .await
        .unwrap();

    // Classic Swanson example: Raynaud's disease
    let discoveries = engine
        .discover_connections("raynauds_disease", 10)
        .await
        .unwrap();
    ...
    let cross_domain_count = discoveries
        .iter()
