...
async fn test_thought_capture() {
    ...
    let hermes = HermesEngine::new(config).await.expect("Failed to create HERMES engine");
    ...
    let insight_id = hermes.capture_thought(input).await.expect("Failed to capture thought");
