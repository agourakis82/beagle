...
impl CitationVerifier {
    ...
    fn authors_match(&self, citation_authors: &[String], matched_authors: &[String]) -> bool {
        ...
        let citation_lower: Vec<String> =
            citation_authors.iter().map(|a| a.to_lowercase()).collect();

        let matched_lower: Vec<String> = matched_authors.iter().map(|a| a.to_lowercase()).collect();

        let matches: usize = citation_lower
            .iter()
            .filter(|ca| {
                matched_lower.iter().any(|ma| {
                    ma.contains(ca.as_str())
                        || ca.contains(ma.as_str())
