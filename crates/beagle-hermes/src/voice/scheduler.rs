...
impl RetrainingScheduler {
    pub fn new(trainer: LoRATrainer, corpus_path: PathBuf, adapter_path: PathBuf) -> Self {
        Self {
            trainer,
            corpus_path,
            adapter_path,
    ...
    pub async fn start(&self) {
        info!(
            "Retraining scheduler started (runs daily at {}:00)",
            self.schedule_hour
        );
    ...
    async fn retrain(&self) -> Result<TrainingMetrics, Box<dyn std::error::Error>> {
        ...
        info!(
            "Collected {} new examples from yesterday",
            new_examples.len()
        );
    ...
    async fn collect_yesterdays_interactions(
        &self,
