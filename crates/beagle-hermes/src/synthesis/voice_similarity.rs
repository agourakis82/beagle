...
impl VoiceSimilarityAnalyzer {
    ...
    pub fn compute_similarity(&self, draft: &str, reference_corpus: &[String]) -> Result<f64> {
        ...
        debug!("Voice similarity computed: {:.2}%", final_similarity * 100.0);
        Ok(final_similarity)
    }
    ...
    fn extract_features(&self, text: &str) -> HashMap<String, f64> {
        ...
        let punct_chars: usize = text.chars().filter(|c| matches!(c, '.' | ',' | ';' | ':' | '!' | '?')).count();
        ...
        let transition_count = text.to_lowercase()
            .split_whitespace()
        ...
        let passive_count = passive_patterns.iter()
        ...
        let common_words: std::collections::HashSet<&str> = ["therefore", "furthermore", "nevertheless", "additionally"].into_iter().collect();
