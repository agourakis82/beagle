...
use crate::{Result, HermesError, knowledge::KnowledgeGraph};
...
impl SynthesisEngine {
    ...
    pub async fn synthesize(&self, request: SynthesisRequest) -> Result<SynthesisResult> {
        info!("Starting synthesis for cluster: {}", request.cluster.concept_name);
    ...
    async fn prepare_context(&self, request: &SynthesisRequest) -> Result<String> {
        ...
        context.push_str(&format!("- Target length: {} words\n", request.target_words));
        context.push_str(&format!("- Voice similarity target: {:.1}%\n", request.voice_profile.similarity_target * 100.0));
...
"#,
        ...
        let response = self.llm_client.complete(llm_request).await
            .map_err(|e| HermesError::SynthesisError(format!("LLM generation failed: {}", e)))?;
    ...
    fn compute_voice_similarity(&self, draft: &str, profile: &VoiceProfile) -> Result<f64> {
        ...
        if similarity >= profile.similarity_target {
            debug!("Voice similarity {:.1}% meets target {:.1}%", 
                similarity * 100.0, profile.similarity_target * 100.0);
        } else {
            debug!("Voice similarity {:.1}% below target {:.1}%", 
                similarity * 100.0, profile.similarity_target * 100.0);
        }
    ...
    pub async fn check_trigger(&self, insight_id: &Uuid, _knowledge_graph: &KnowledgeGraph) -> Result<()> {
