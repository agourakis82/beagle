...
use super::{InsightNode, RelationType};
use crate::{Result, HermesError, thought_capture::Insight};
...
impl KnowledgeGraph {
    ...
    async fn setup_schema(graph: &Graph) -> Result<()> {
        ...
        graph.run(query(
            ...
            .map_err(|e| HermesError::Neo4jError(format!("Failed to create concept index: {}", e)))?;
    ...
    pub async fn store_insight(&self, insight: &Insight) -> Result<Uuid> {
        ...
        let mut result = (*self.graph)
            .execute(
                query(query_str)
                    ...
                    .param("content", insight.content.clone())
                    .param("timestamp", insight.timestamp.to_rfc3339())
                    .param("context", format!("{:?}", insight.context))
        ...
        for concept in &insight.concepts {
            self.link_concept(insight.id, concept).await?;
        }

        // 3. Create Entity nodes and relationships
        for entity in &insight.entities {
            self.link_entity(insight.id, &entity.text, &format!("{:?}", entity.entity_type)).await?;
        }
    ...
    async fn link_entity(&self, insight_id: Uuid, entity_text: &str, entity_type: &str) -> Result<()> {
        let query_str = "
            MATCH (i:Insight {id: $insight_id})
        ...
        (*self.graph)
            .run(
                query(query_str)
                    .param("insight_id", insight_id.to_string())
                    .param("entity_text", entity_text)
                    .param("entity_type", entity_type)
            )
            .await
            .map_err(|e| HermesError::Neo4jError(format!("Failed to link entity: {}", e)))?;

        Ok(())
    }

    async fn detect_relationships(&self, insight: &Insight) -> Result<()> {
        ...
        (*self.graph)
            .run(
                query(query_str)
                    .param("new_id", insight.id.to_string())
            )
            .await
            .map_err(|e| HermesError::Neo4jError(format!("Failed to detect relationships: {}", e)))?;

        Ok(())
    }

    /// Find dense concept clusters (candidates for synthesis)
    pub async fn find_concept_clusters(&self, min_insights: usize) -> Result<Vec<super::ConceptCluster>> {
        let query_str = "
        ...
        let mut result = (*self.graph)
            .execute(
                query(query_str)
                    .param("min_insights", min_insights as i64)
            )
            .await
        ...
        while let Some(row) = result.next().await
            ...
            let concept_name: String = row.get("concept_name")
                .map_err(|e| HermesError::Neo4jError(format!("Failed to get concept_name: {}", e)))?;
            let insight_count: i64 = row.get("insight_count")
                .map_err(|e| HermesError::Neo4jError(format!("Failed to get insight_count: {}", e)))?;

            // Parse insights_data from Neo4j result
            let insights_data: serde_json::Value = row.get("insights_data")
                .map_err(|e| HermesError::Neo4jError(format!("Failed to get insights_data: {}", e)))?;
