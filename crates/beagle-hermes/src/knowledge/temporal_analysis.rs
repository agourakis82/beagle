...
impl TemporalAnalyzer {
    ...
    pub fn analyze_growth_rate(
        ...
        let time_span = (last - first).num_days().max(1);

        // Growth rate: insights per day
        let growth_rate = insights.len() as f64 / time_span as f64;
        debug!("Growth rate: {:.2} insights/day over {} days", growth_rate, time_span);
    ...
    pub fn detect_emerging_themes(
        ...
        for cluster in concept_clusters {
            ...
            if growth_rate > 0.5 { // More than 0.5 insights per day
                emerging.push(cluster.concept_name.clone());
                debug!("Emerging theme detected: {} (growth: {:.2}/day)", 
    ...
    pub fn find_temporal_patterns(
        ...
        let peak_hour = hourly_distribution.iter()
            .max_by_key(|(_, &count)| count)
            .map(|(&hour, _)| hour)
            .unwrap_or(12);

        let peak_day = daily_distribution.iter()
            .max_by_key(|(_, &count)| count)
            .map(|(&day, _)| day)
            .unwrap_or(1);
