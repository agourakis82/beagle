...
impl ArgosAgent {
    ...
    pub async fn validate(&self, draft: &Draft, papers: &[Paper]) -> Result<ValidationResult> {
        ...
        let quality_score = self.compute_quality_score(
            citation_validity.completeness,
            flow_score,
            issues.len(),
        );
    ...
    fn check_transitions(&self, content: &str) -> Vec<Issue> {
        ...
        for i in 1..paragraphs.len() {
            ...
            let has_transition = transition_words.iter()
                .any(|&word| curr_para.starts_with(word) || curr_para.contains(&format!(", {}", word)));
            ...
            if !has_transition && shared_words.len() < 3 && prev_para.len() > 50 && curr_para.len() > 50 {
                issues.push(Issue {
    ...
    fn check_unclear_references(&self, content: &str) -> Vec<Issue> {
        ...
        for (i, sentence) in sentences.iter().enumerate() {
            ...
            for pronoun in &unclear_pronouns {
                if lower.contains(&format!(" {} ", pronoun)) || lower.starts_with(&format!("{} ", pronoun)) {
                    ...
                    if i > 0 {
                        ...
                        let has_nouns = prev_sentence.split_whitespace()
...
impl QualityScorer {
    ...
    fn analyze_flow(&self, content: &str) -> Result<f64> {
        ...
        let transition_words = [
            ...
            "therefore", "thus", "hence", "nevertheless", "nonetheless",
            "in contrast", "on the other hand", "similarly", "likewise",
            "firstly", "secondly", "finally", "in conclusion", "further",
        ...
        let evidence_indicators = ["data", "results", "analysis", "study", "experiment", "finding"];

        let mut claim_evidence_balance = 0.0;
        let claim_count = claim_indicators.iter()
