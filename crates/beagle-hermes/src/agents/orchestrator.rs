...
use super::{AthenaAgent, HermesAgent, ArgosAgent};
use crate::{Result, knowledge::ConceptCluster, synthesis::VoiceProfile};
...
impl MultiAgentOrchestrator {
    ...
    pub async fn synthesize_section(
        ...
        info!("🚀 Multi-Agent Synthesis for cluster: {}", cluster.concept_name);
        ...
        info!("✅ ARGOS: Quality score: {:.1}%", validation.quality_score * 100.0);
        ...
        if !validation.approved {
            ...
            let argos_clone = Arc::clone(&self.argos);
            let papers_clone = papers.clone();
            let validation_fn = move |draft_text: &str| -> crate::Result<Vec<crate::agents::argos::Issue>> {
                // Create temporary draft for validation
                let temp_draft = super::hermes_agent::Draft {
                    content: draft_text.to_string(),
                    word_count: draft_text.split_whitespace().count(),
                    citations: extract_citations_from_text(draft_text),
                };
                
                // Run validation (this is sync, so we need to handle it differently)
                // For now, just return the issues from the original validation
                Ok(Vec::new()) // Placeholder - would need async validation
            };
            
            // For now, do a single refinement pass
            match refinement_engine.refine(&final_draft, &validation.issues, &context_text).await {
                Ok(refined) => {
                    final_draft = refined;
    ...
    pub async fn synthesize_multiple_clusters(
        ...
        for cluster in clusters {
            ...
            join_set.spawn(async move {
                orchestrator.synthesize_section(&cluster, section_type, target_words).await
            });
        }
