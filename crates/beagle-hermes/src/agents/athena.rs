...
use crate::{Result, knowledge::ConceptCluster};
use beagle_hypergraph::{CachedPostgresStorage, RAGPipeline, LanguageModel};
use beagle_hypergraph::rag::LanguageModelError;
use beagle_hypergraph::embeddings::{EmbeddingGenerator, OpenAIEmbeddings, MockEmbeddingGenerator};
...
impl AthenaAgent {
    pub async fn new() -> Result<Self> {
        ...
        let rag_pipeline = match Self::try_init_rag_pipeline(Arc::clone(&llm_arc)).await {
            ...
            Err(e) => {
                warn!("ATHENA: Failed to initialize RAG pipeline: {}. Using fallback search.", e);
                None
            }
        };
    ...
    async fn try_init_rag_pipeline(
        ...
        let storage = Arc::new(
            CachedPostgresStorage::new(&postgres_uri, &redis_uri).await
                .map_err(|e| crate::HermesError::DatabaseError(
                    sqlx::Error::Configuration(format!("Hypergraph error: {}", e).into())
                ))?
        );

        // Create language model wrapper
        let language_model: Arc<dyn LanguageModel> = Arc::new(AnthropicLanguageModel {
            client: llm,
        });

        // Initialize embedding generator
        // Try OpenAI first, fallback to Mock if API key not available
        let embeddings: Arc<dyn EmbeddingGenerator> = if let Ok(openai_key) = std::env::var("OPENAI_API_KEY") {
        ...
        let pipeline = Arc::new(
            RAGPipeline::new(storage, language_model, embeddings)
        );
    ...
    pub async fn search_papers(&self, cluster: &ConceptCluster) -> Result<Vec<Paper>> {
        info!("ATHENA: Searching papers for cluster: {}", cluster.concept_name);
...
"#,
        ...
        // Parse JSON response
        let extracted_papers: Vec<Paper> = serde_json::from_str(&response.content)
            .map_err(|e| {
                warn!("Failed to parse extracted papers JSON: {}. Response: {}", e, response.content);
...
"#,
        ...
        let request = CompletionRequest {
            ...
            system: Some("You are an expert at finding relevant scientific literature.".to_string()),
        };
        ...
        // Parse JSON response
        let papers: Vec<Paper> = serde_json::from_str(&response.content)
            .map_err(|e| crate::HermesError::SynthesisError(format!("Failed to parse papers JSON: {}", e)))?;
    ...
    pub async fn extract_key_findings(&self, papers: &[Paper]) -> Result<Vec<Finding>> {
        ...
        info!("ATHENA: Extracting key findings from {} papers", papers.len());

        // Prepare paper summaries for LLM
        let papers_text: String = papers.iter()
            .enumerate()
            .map(|(i, paper)| {
                format!(
                    "Paper {}: {}\nAuthors: {}\nYear: {}\nAbstract: {}\n",
...
"#,
        ...
        let parsed: Vec<FindingResponse> = serde_json::from_str(&response.content)
            .map_err(|e| crate::HermesError::SynthesisError(format!("Failed to parse findings JSON: {}", e)))?;
