...
use beagle_quantum::{
    ...
    CollapseStrategy, HypothesisSet,
};

#[tokio::test]
async fn test_superposition_generation() {
    let agent = SuperpositionAgent::new();
    let set = agent.generate_hypotheses("Como explicar a curvatura da entropia em scaffolds?").await;
    
    // Se vLLM não estiver disponível, o teste pode falhar - isso é esperado
    match set {
        Ok(set) => {
            assert!(!set.hypotheses.is_empty(), "Deve gerar pelo menos uma hipótese");
            // Verifica que as probabilidades somam ~1.0
            let total_prob: f64 = set.hypotheses.iter().map(|h| h.confidence).sum();
            assert!((total_prob - 1.0).abs() < 0.1, "Probabilidades devem somar ~1.0, soma: {}", total_prob);
        }
        Err(e) => {
            // Se vLLM não estiver disponível, apenas loga o erro
            eprintln!("⚠️  vLLM não disponível (esperado em testes sem cluster): {}", e);
...
async fn test_interference_constructive() {
    ...
    set.add("Hipótese A: Entropia aumenta com temperatura".to_string(), Some((0.5, 0.1)));
    set.add("Hipótese B: Entropia diminui com temperatura".to_string(), Some((0.3, 0.1)));
    
    // Encontrar qual hipótese corresponde a A antes da interferência
    let initial_a_idx = set.hypotheses.iter()
    ...
    match engine.apply_evidence(&mut set, "Evidência experimental confirma que entropia aumenta com temperatura", 1.0).await {
        Ok(_) => {
        ...
        Err(e) => {
            // Se embedding server não estiver disponível, apenas loga
            eprintln!("⚠️  Embedding server não disponível (esperado em testes sem cluster): {}", e);
        }
    }
}
...
async fn test_measurement_greedy() {
    ...
    let result = operator.measure(set, CollapseStrategy::Greedy).await.unwrap();
    
    assert_eq!(result, "Hipótese 1", "Greedy deve retornar a melhor hipótese");
}
...
async fn test_measurement_probabilistic() {
    ...
    for _ in 0..100 {
        let result = operator.measure(set.clone(), CollapseStrategy::Probabilistic).await.unwrap();
...
async fn test_full_quantum_pipeline() {
    ...
    let set_result = quantum.generate_hypotheses("Como explicar a curvatura da entropia em scaffolds?").await;
    ...
    // Se embedding server não estiver disponível, apenas loga
    if let Err(e) = interference.apply_evidence(&mut set, "Evidência experimental 2024 confirma modelo quântico de campo", 1.0).await {
        eprintln!("⚠️  Embedding server não disponível (esperado em testes sem cluster): {}", e);
    }
    
    let measurement = MeasurementOperator::new();
    let final_answer = measurement.measure(set, CollapseStrategy::Probabilistic).await.unwrap();
    
    assert!(!final_answer.is_empty(), "Resposta final não deve estar vazia");
    println!("✅ Pipeline completo: {}", &final_answer[..final_answer.len().min(80)]);
}

#[tokio::test]
async fn test_measurement_critic_guided() {
    let mut set = HypothesisSet::new();
    set.add("Hipótese 1: Abordagem clássica newtoniana".to_string(), Some((0.6, 0.1)));
    set.add("Hipótese 2: Modelo quântico de campo".to_string(), Some((0.4, 0.1)));
    set.add("Hipótese 3: Interpretação geométrica".to_string(), Some((0.3, 0.1)));
    ...
    match operator.collapse(set, CollapseStrategy::CriticGuided).await {
        Ok(result) => {
            assert!(!result.is_empty(), "CriticGuided deve retornar resposta não vazia");
            println!("✅ CriticGuided colapsou: {}", &result[..result.len().min(100)]);
        }
        Err(e) => {
            eprintln!("⚠️  vLLM não disponível para CriticGuided (esperado em testes sem cluster): {}", e);
        }
    }
}
