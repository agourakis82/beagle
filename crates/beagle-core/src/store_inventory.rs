...
impl BeagleStoreInventory {
    ...
    ///
    /// Also emits an INFO confirming the pgvector dimension is reconciled: migration
    /// 003_embedding_dim_1024.sql altered `nodes.embedding` from VECTOR(1536) to VECTOR(1024)
    /// to match every live index (Qdrant + LanceDB, all 1024-dim bge-m3/jina-colbert-v2), so
    /// the ADR 0001 §4.3 rebuild-from-Postgres dimension is now valid. The migration was run
    /// against the live DB gated by the darwin-eval parity baseline (no retrieval regression).
    /// The column is still an empty cold backup — backfilling it (and rebuilding the ivfflat
    /// index) is the remaining ADR 0001 §DEFERRED work before a Postgres-seeded rebuild is real.
    pub fn log(&self) {
        ...
        // DIM RECONCILED (migration 003_embedding_dim_1024.sql, gated by darwin-eval parity):
        // nodes.embedding is now VECTOR(1024), matching every live index (Qdrant
        // beagle_exocortex/cockpit_rag + LanceDB semantic_memory_v1, all 1024-dim
        // bge-m3/jina-colbert-v2). The §4.3 rebuild-from-Postgres dimension is now valid.
        // NOTE: the column is currently an EMPTY cold backup (0 embeddings) — populate it
        // (backfill) and rebuild the ivfflat index before relying on a Postgres-seeded rebuild.
        info!(
            "pgvector dim reconciled to 1024 (migration 003) — matches live indexes \
             (bge-m3/jina-colbert-v2); §4.3 rebuild dimension now valid. Column is an empty \
             cold backup — backfill + reindex before relying on Postgres-seeded rebuild."
        );
    }
