...
impl NeuralEngine {
    ...
    pub async fn train_lora_native(&self, bad_draft: &str, good_draft: &str) -> Result<()> {
        if !self.device_available {
            return Err(anyhow::anyhow!("Neural Engine não disponível — MLX não encontrado"));
        }
        ...
        std::fs::write(bad_path, bad_draft)
            .context("Falha ao salvar bad_draft")?;
        std::fs::write(good_path, good_draft)
        ...
        let status = Command::new("julia")
            ...
            .env("OUTPUT_DIR", "/home/agourakis82/beagle-data/lora/current_voice")
            .status()
    ...
    async fn train_lora_mlx_python(&self, bad_path: &str, good_path: &str) -> Result<()> {
        ...
        if !Path::new(python_script).exists() {
            return Err(anyhow::anyhow!("Script MLX não encontrado: {}", python_script));
        }
    ...
    pub async fn embed_local(&self, text: &str) -> Result<Vec<f32>> {
        ...
        let embedding: Vec<f32> = serde_json::from_str(&json_str)
            .context("Falha ao parsear embedding JSON")?;
    ...
    async fn whisper_fallback(&self, _audio_path: &str) -> Result<String> {
        Err(anyhow::anyhow!("whisper.cpp não encontrado e beagle-whisper não disponível"))
    }
