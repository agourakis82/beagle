...
use beagle_bilingual::{to_bilingual, generate_bilingual_thread};
...
impl BeagleTwitter {
    ...
    pub async fn auto_post_bilingual_thread(
        ...
        info!("✅ Thread bilíngue completa postada — {} tweets", tweet_ids.len());
        Ok(tweet_ids)
    }
...
pub async fn auto_post_if_ready(
    ...
    let bearer_token = std::env::var("TWITTER_BEARER_TOKEN")
        .context("TWITTER_BEARER_TOKEN não configurado")?;
    
    let twitter = BeagleTwitter::new(bearer_token);
    let tweet_ids = twitter.auto_post_bilingual_thread(title_pt, abstract_pt, paper_url).await?;
