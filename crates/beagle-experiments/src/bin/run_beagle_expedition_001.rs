...
use beagle_experiments::{
    ...
    exp001::{
        assert_expedition_001_llm_config,
        Expedition001Config,
        EXPEDITION_001_ID,
        EXPEDITION_001_DEFAULT_N,
        triad_condition_flags,
        single_condition_flags,
    },
    ExperimentRunTag,
};
use chrono::Utc;
...
async fn main() -> Result<()> {
    ...
    let config = Expedition001Config::with_custom(
        args.experiment_id,
        args.n_total,
    ...
    if n_triad != n_single {
        warn!("⚠️  Número ímpar de runs: diferença de {} entre condições (aceitável)", 
              (n_triad as i32 - n_single as i32).abs());
    }
    
    info!("   Condições: triad (N={}) vs single (N={})", n_triad, n_single);
    ...
    let (triad_enabled, hrv_aware_triad, serendipity_enabled, space_aware) = triad_condition_flags();
    ...
    for (i, condition) in conditions.iter().enumerate() {
        let question = if config.question_template.contains("{i}") {
            config.question_template.replace("{i}", &(i + 1).to_string())
        } else {
            format!("{} (run {}/{}, batch={})", 
        ...
        let (with_triad, hrv_aware) = match condition.as_str() {
            ...
            _ => {
                warn!("Condição desconhecida: {}, usando triad como fallback", condition);
    ...
    println!("\nExperiment tags written to: {}/experiments/events.jsonl", data_dir.display());
    println!("\nNext steps:");
    ...
    println!("3. Analyze results: analyze_experiments {} --output-format csv", config.experiment_id);
    println!();
    
    info!("🎉 Expedition 001 concluída!");
    info!("   Execute 'analyze_experiments {}' para analisar resultados", config.experiment_id);
