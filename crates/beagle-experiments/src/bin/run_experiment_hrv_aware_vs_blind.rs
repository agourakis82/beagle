...
struct Cli {
    ...
    #[arg(
        long,
        default_value = "Paper idea {i}: Explorar efeitos do estado fisiológico em síntese científica"
    )]
...
async fn main() -> Result<()> {
    ...
    info!(
        "📊 Fase 1: Executando {} runs com HRV-aware ENABLED",
        n_per_condition
    );
    ...
    info!(
        "📊 Fase 2: Executando {} runs com HRV-aware DISABLED",
        n_per_condition
    );
    for i in 0..n_per_condition {
        let question = args
