...
fn main() -> anyhow::Result<()> {
    ...
    let (triad_enabled, hrv_aware, serendipity_enabled, space_aware) = 
        infer_flags_from_run_report(&data_dir, &args.run_id)
        .unwrap_or_else(|| {
...
fn infer_flags_from_run_report(data_dir: &PathBuf, run_id: &str) -> Option<(bool, bool, bool, bool)> {
                    ...
                    if let Ok(content) = std::fs::read_to_string(entry.path()) {
                        if let Ok(report) = serde_json::from_str::<serde_json::Value>(&content) {
                            ...
                            let triad_enabled = report.get("triad_report_json").is_some() ||
                                data_dir.join("triad").join(run_id).join("draft_reviewed.md").exists();
                            
                            // Infere hrv_aware: se há observer.physio_severity no report
                            let hrv_aware = report.get("observer")
                            ...
                            let space_aware = report.get("observer")
                                .and_then(|o| o.get("space_severity"))
                                .and_then(|s| s.as_str())
                                .map(|s| s != "Normal")
                                .unwrap_or(false);
                            
                            return Some((triad_enabled, hrv_aware, serendipity_enabled, space_aware));
                        }
                    }
                }
            }
