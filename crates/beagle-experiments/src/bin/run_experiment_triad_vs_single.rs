...
struct Cli {
    ...
    #[arg(
        long,
        default_value = "Paper idea {i}: Explorar aplicações de scaffolds biológicos em medicina regenerativa"
    )]
...
async fn main() -> Result<()> {
    ...
    info!(
        "📊 Fase 1: Executando {} runs com Triad ENABLED",
        n_per_condition
    );
    ...
    info!(
        "📊 Fase 2: Executando {} runs com Triad DISABLED",
        n_per_condition
    );
    for i in 0..n_per_condition {
        let question = args
