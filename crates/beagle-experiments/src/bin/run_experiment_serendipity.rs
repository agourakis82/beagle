...
struct Cli {
    ...
    #[arg(long, default_value = "Paper idea {i}: Explorar conexões interdisciplinares em síntese científica")]
...
async fn main() -> Result<()> {
    ...
    info!("📊 Fase 2: Executando {} runs com Serendipity DISABLED", n_per_condition);
    env::set_var("BEAGLE_SERENDIPITY", "false");
    
    for i in 0..n_per_condition {
        let question = args.question_template.replace("{i}", &(i + 1 + n_per_condition).to_string());
        info!("Run {}/{} (serendipity_off): {}", i + 1, n_per_condition, question);
