...
use beagle_feedback::{append_event, create_pipeline_event, create_human_feedback_event};
...
async fn test_expedition_001_tagging_and_analysis() -> anyhow::Result<()> {
    ...
    for tag in &tags {
        if tag.condition == "triad" {
            ...
            assert!(!tag.triad_enabled, "Tags single devem ter triad_enabled=false");
        }
        assert!(tag.hrv_aware, "Todas as tags devem ter hrv_aware=true (Expedition 001)");
        assert!(!tag.serendipity_enabled, "Todas as tags devem ter serendipity_enabled=false (Expedition 001)");
        assert!(!tag.space_aware, "Todas as tags devem ter space_aware=false (Expedition 001)");
    }
    ...
    assert!(delta > 0.0, "Triad deve ter rating maior que Single (effect positivo)");
    ...
    assert_eq!(single_metrics.physio_severity_counts.get("Normal"), Some(&5));
    ...
    println!("   Triad rating mean: {:.2}", triad_metrics.rating_mean.unwrap());
    println!("   Single rating mean: {:.2}", single_metrics.rating_mean.unwrap());
