...
LOCAL_PORT="${LOCAL_PORT:-18099}"
OUT="${OUT:-${ROOT}/.artifacts/darwin-hpc/grok-bridge-smoke}"
OPERATOR_API_TOKEN="${BEAGLE_OPERATOR_API_TOKEN:-${BEAGLE_API_TOKEN:-}}"
SECRET_NAME="${SECRET_NAME:-beagle-core-secrets}"
REAL_PROVIDER="${REAL_PROVIDER:-grok_fast}"
REAL_MODEL="${REAL_MODEL:-grok-4-1-fast-reasoning}"
REQUEST_ID="${REQUEST_ID:-b122c3-grok-$(date +%m%d%H%M%S)}"
...
ensure_xai_secret() {
  if resolve_secret_value "XAI_API_KEY" >/dev/null; then
    return 0
  fi

  echo "[FAIL] XAI_API_KEY is not materialized in ${SECRET_NAME}" >&2
  exit 1
}
...
if rg -q "execute_grok_fast|XAI_API_KEY|grok_fast request failed|grok_fast returned" "${SOURCE_FILE}"; then
  SOURCE_EXECUTE_PRESENT=1
fi
if rg -q 'provider: BridgeProvider::GrokFast|implemented: true|cheap API lane via xAI' "${SOURCE_FILE}"; then
  SOURCE_PROVIDER_PRESENT=1
fi
if rg -q 'XAI_API_KEY' "${SECRET_FILE}"; then
...
cat > "${OUT}/execute-grok-request.json" <<EOF
{
  ...
  "payload": {
    "input": "Extract the single core architectural rule from this sentence in one short bullet: Beagle should add Grok fast through the existing cheap provider bridge path without redesigning the bridge foundation."
  },
  "metadata": {
    "source": "run_grok_bridge_smoke"
  }
}
EOF

curl_json POST "/api/darwin/bridge/execute" "${OUT}/execute-grok.json" "${OUT}/execute-grok-request.json"
...
EXECUTE_STATUS="$(jq -r '.status // empty' "${OUT}/execute-grok.json")"
EXECUTE_PROVIDER="$(jq -r '.provider // empty' "${OUT}/execute-grok.json")"
EXECUTE_MODEL="$(jq -r '.model // empty' "${OUT}/execute-grok.json")"
EXECUTE_ERROR="$(jq -r '.error // empty' "${OUT}/execute-grok.json")"
...
echo "[OK] grok bridge smoke artifacts written to ${OUT}"
