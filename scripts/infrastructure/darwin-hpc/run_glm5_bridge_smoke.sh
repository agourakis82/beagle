...
OUT="${OUT:-${ROOT}/.artifacts/darwin-hpc/glm5-bridge-smoke}"
OPERATOR_API_TOKEN="${BEAGLE_OPERATOR_API_TOKEN:-${BEAGLE_API_TOKEN:-}}"
SECRET_NAME="${SECRET_NAME:-beagle-core-secrets}"
REAL_PROVIDER="${REAL_PROVIDER:-glm5}"
REAL_MODEL="${REAL_MODEL:-glm-5}"
REQUEST_ID="${REQUEST_ID:-b122c1-glm5-$(date +%m%d%H%M%S)}"
...
ensure_glm5_secret() {
  if resolve_secret_value "ZAI_API_KEY" >/dev/null; then
    return 0
  fi

  echo "[FAIL] ZAI_API_KEY is not materialized in ${SECRET_NAME}" >&2
  exit 1
}
...
if rg -q "execute_glm5|ZAI_API_KEY|glm5 request failed|glm5 returned" "${SOURCE_FILE}"; then
  SOURCE_EXECUTE_PRESENT=1
fi
if rg -q 'provider: BridgeProvider::Glm5|implemented: true|cheap API lane via ZAI' "${SOURCE_FILE}"; then
  SOURCE_PROVIDER_PRESENT=1
fi
if rg -q 'ZAI_API_KEY' "${SECRET_FILE}"; then
...
cat > "${OUT}/execute-glm5-request.json" <<EOF
{
  ...
  "payload": {
    "input": "Extract the single core architectural rule from this sentence in one short bullet: Beagle should add GLM-5 through the existing cheap provider bridge path without redesigning the bridge foundation."
  },
  "metadata": {
    "source": "run_glm5_bridge_smoke"
  }
}
EOF

curl_json POST "/api/darwin/bridge/execute" "${OUT}/execute-glm5.json" "${OUT}/execute-glm5-request.json"
...
EXECUTE_PROVIDER="$(jq -r '.provider // empty' "${OUT}/execute-glm5.json")"
EXECUTE_MODEL="$(jq -r '.model // empty' "${OUT}/execute-glm5.json")"
...
echo "[OK] glm5 bridge smoke artifacts written to ${OUT}"
