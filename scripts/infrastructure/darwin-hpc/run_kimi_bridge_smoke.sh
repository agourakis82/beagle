...
OUT="${OUT:-${ROOT}/.artifacts/darwin-hpc/kimi-bridge-smoke}"
OPERATOR_API_TOKEN="${BEAGLE_OPERATOR_API_TOKEN:-${BEAGLE_API_TOKEN:-}}"
SECRET_NAME="${SECRET_NAME:-beagle-core-secrets}"
REAL_PROVIDER="${REAL_PROVIDER:-kimi}"
REAL_MODEL="${REAL_MODEL:-moonshotai/kimi-k2-instruct-0905}"
REQUEST_ID="${REQUEST_ID:-b122c4-kimi-$(date +%m%d%H%M%S)}"
...
ensure_groq_secret() {
  if resolve_secret_value "GROQ_API_KEY" >/dev/null; then
    return 0
  fi

  echo "[FAIL] GROQ_API_KEY is not materialized in ${SECRET_NAME}" >&2
  exit 1
}
...
if rg -q "execute_kimi|GROQ_API_KEY|kimi request failed|kimi returned" "${SOURCE_FILE}"; then
  SOURCE_EXECUTE_PRESENT=1
fi
if rg -q 'provider: BridgeProvider::Kimi|implemented: true|cheap API lane via Groq' "${SOURCE_FILE}"; then
  SOURCE_PROVIDER_PRESENT=1
fi
if rg -q 'GROQ_API_KEY' "${SECRET_FILE}"; then
...
cat > "${OUT}/execute-kimi-request.json" <<EOF
{
  ...
  "payload": {
    "input": "Extract the single core architectural rule from this sentence in one short bullet: Beagle should add Kimi through the existing cheap provider bridge path without redesigning the bridge foundation."
  },
  "metadata": {
    "source": "run_kimi_bridge_smoke"
  }
}
EOF

curl_json POST "/api/darwin/bridge/execute" "${OUT}/execute-kimi.json" "${OUT}/execute-kimi-request.json"
...
EXECUTE_PROVIDER="$(jq -r '.provider // empty' "${OUT}/execute-kimi.json")"
EXECUTE_MODEL="$(jq -r '.model // empty' "${OUT}/execute-kimi.json")"
...
echo "[OK] kimi bridge smoke artifacts written to ${OUT}"
