...
OUT="${OUT:-${ROOT}/.artifacts/darwin-hpc/study-promotion-execution}"
...
DOC_FILE="${DOC_FILE:-${ROOT}/docs/darwin/hpc/B265_STUDY_CLOSEOUT_AND_CANONICAL_VARIANT_PROMOTION_EXECUTION.md}"
GO_NO_GO_FILE="${GO_NO_GO_FILE:-${ROOT}/docs/darwin/hpc/B265_GO_NO_GO.md}"
KNOWN_LIMITS_FILE="${KNOWN_LIMITS_FILE:-${ROOT}/docs/darwin/hpc/B265_KNOWN_LIMITS.md}"
...
STUDY_PROMOTION_EXECUTION_SOURCE_FILE="${STUDY_PROMOTION_EXECUTION_SOURCE_FILE:-${ROOT}/crates/beagle-darwin/src/study_promotion_execution.rs}"
STUDY_PROMOTION_EXECUTION_SCHEMA_FILE="${STUDY_PROMOTION_EXECUTION_SCHEMA_FILE:-${ROOT}/docs/darwin/hpc/contracts/study-promotion-execution-schema.yaml}"
...
SOURCE_PROMOTION_EXECUTION_PRESENT=0
DOC_PRESENT=0
GO_NO_GO_PRESENT=0
...
SCHEMA_PROMOTION_EXECUTION_PRESENT=0
B263_BASE_PRESENT=0
B262_BASE_PRESENT=0
B261_BASE_PRESENT=0
...
if rg -q "StudyPromotionExecution|ensure_study_promotion_execution|study-promotion-execution" "${STUDY_PROMOTION_EXECUTION_SOURCE_FILE}"; then
  SOURCE_PROMOTION_EXECUTION_PRESENT=1
fi
...
[[ -f "${STUDY_PROMOTION_EXECUTION_SCHEMA_FILE}" ]] && SCHEMA_PROMOTION_EXECUTION_PRESENT=1
...
jq -n \
  ...
  --argjson source_promotion_execution_present "${SOURCE_PROMOTION_EXECUTION_PRESENT}" \
  ...
  --argjson schema_promotion_execution_present "${SCHEMA_PROMOTION_EXECUTION_PRESENT}" \
  ...
  '{
    ...
    source_promotion_execution_present: $source_promotion_execution_present,
    ...
    schema_promotion_execution_present: $schema_promotion_execution_present,
...
[[ "${SOURCE_PROMOTION_EXECUTION_PRESENT}" == "1" ]] || { echo "[FAIL] promotion execution source missing" >&2; exit 1; }
...
[[ "${SCHEMA_PROMOTION_EXECUTION_PRESENT}" == "1" ]] || { echo "[FAIL] promotion execution schema missing" >&2; exit 1; }
...
FETCH_PATH="/api/darwin/workstreams/${WORKSTREAM_ID}/study-promotion-execution"
BUNDLE_BEFORE="${OUT}/study-promotion-execution-bundle-before.json"
BUNDLE_AFTER="${PROOF_OUT}/study-promotion-execution-bundle-after-restart.json"
...
jq '.study_promotion_execution' "${BUNDLE_BEFORE}" > "${OUT}/study-promotion-execution.json"
...
jq '.study_promotion_execution' "${BUNDLE_AFTER}" > "${PROOF_OUT}/study-promotion-execution-after-restart.json"

jq -n \
  --argjson before "$(jq -c '.' "${BUNDLE_BEFORE}")" \
  --argjson after "$(jq -c '.' "${BUNDLE_AFTER}")" \
  --argjson execution "$(jq -c '.study_promotion_execution' "${BUNDLE_BEFORE}")" \
  '{
    phase: "B26.5",
    ...
    promoted_variant_id: $execution.promoted_variant_id,
    promoted_run_id: $execution.promoted_run_id,
    archived_variant_count: $execution.archived_variant_count,
    promoted_run_score: $execution.promoted_run_score,
    restart_recovered_identity: (
      $before.study_promotion_execution.study_id == $after.study_promotion_execution.study_id and
      $before.study_promotion_execution.workspace_id == $after.study_promotion_execution.workspace_id and
      $before.study_promotion_execution.session_id == $after.study_promotion_execution.session_id and
      $before.study_promotion_execution.same_beagle_owned_identity == $after.study_promotion_execution.same_beagle_owned_identity
    ),
    restart_recovered_summary: (
      $before.study_promotion_execution.promoted_variant_id == $after.study_promotion_execution.promoted_variant_id and
      $before.study_promotion_execution.promoted_run_id == $after.study_promotion_execution.promoted_run_id
    )
  }' > "${OUT}/smoke.json"
