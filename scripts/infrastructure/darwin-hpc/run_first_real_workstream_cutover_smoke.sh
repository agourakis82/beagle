...
LOCAL_PORT="${LOCAL_PORT:-18097}"
PROFILE_ID="${PROFILE_ID:-gpu-single-v1}"
OUT="${OUT:-${ROOT}/.artifacts/darwin-hpc/first-real-workstream-cutover}"
OPERATOR_API_TOKEN="${BEAGLE_OPERATOR_API_TOKEN:-${BEAGLE_API_TOKEN:-}}"
SECRET_NAME="${SECRET_NAME:-beagle-core-secrets}"
WORKSPACE_ID="${WORKSPACE_ID:-b141-$(date +%m%d%H%M%S)}"
...
EXPECTED_CUTOVER_STATE="${EXPECTED_CUTOVER_STATE:-canonical}"
EXPECTED_BRANCH_LINEAGE="${EXPECTED_BRANCH_LINEAGE:-feat/darwin-hpc-governance}"
...
EXPECTED_EXECUTION_NODE="${EXPECTED_EXECUTION_NODE:-r740-proxmox}"
RUN_LABEL="${RUN_LABEL:-b141-$(date +%m%d%H%M%S)-governance-cutover}"
TIMEOUT_SECONDS="${TIMEOUT_SECONDS:-1200}"
IMAGE_REF="${IMAGE_REF:-localhost/beagle-core:dev}"
ENABLE_OPTIONAL_FALLBACK="${ENABLE_OPTIONAL_FALLBACK:-0}"
FALLBACK_REASON="${FALLBACK_REASON:-first_real_workstream_optional_vm_fallback}"
RETURN_REASON="${RETURN_REASON:-first_real_workstream_return_to_canonical}"
...
echo "${EXPECTED_CUTOVER_STATE}" > "${OUT}/expected-cutover-state.txt"
...
echo "${EXPECTED_EXECUTION_NODE}" > "${OUT}/expected-execution-node.txt"
...
WORKSTREAM_SOURCE_PRESENT=0
CONFIG_MODEL_PRESENT=0
CONFIG_LIB_PRESENT=0
CONFIGMAP_PRESENT=0

if rg -q "WorkspaceWorkstreamCutoverPolicy|workstream_cutover_policy" "${WORKSPACE_SOURCE_FILE}"; then
  WORKSTREAM_SOURCE_PRESENT=1
fi
if rg -q "cutover_workstream|cutover_state|cutover_branch_lineage" "${CONFIG_MODEL_FILE}"; then
  CONFIG_MODEL_PRESENT=1
fi
if rg -q "BEAGLE_WORKSPACE_CUTOVER_WORKSTREAM|BEAGLE_WORKSPACE_CUTOVER_STATE|BEAGLE_WORKSPACE_CUTOVER_BRANCH_LINEAGE" "${CONFIG_LIB_FILE}"; then
  CONFIG_LIB_PRESENT=1
fi
if rg -q "BEAGLE_WORKSPACE_CUTOVER_WORKSTREAM: ${EXPECTED_WORKSTREAM}" "${CONFIG_FILE}" \
  && rg -q "BEAGLE_WORKSPACE_CUTOVER_STATE: ${EXPECTED_CUTOVER_STATE}" "${CONFIG_FILE}" \
  && rg -q "BEAGLE_WORKSPACE_CUTOVER_BRANCH_LINEAGE: ${EXPECTED_BRANCH_LINEAGE}" "${CONFIG_FILE}"; then
  CONFIGMAP_PRESENT=1
fi
...
jq -nc \
  ...
  --argjson workstream_source_present "${WORKSTREAM_SOURCE_PRESENT}" \
  ...
  '{
    ...
    expected_cutover_state: $expected_cutover_state,
...
EOF
  ...
  jq -nc '{observed: false}' > "${OUT}/fallback-summary.json"
fi
...
jq -nc \
  ...
  '{
    ...
    cutover_state: ($bootstrap[0].workstream_cutover_policy.cutover_state // ""),
    branch_lineage: ($bootstrap[0].workstream_cutover_policy.branch_lineage // ""),
    recovery_required: ($bootstrap[0].workstream_cutover_policy.recovery_required // false),
    handoff_required: ($bootstrap[0].workstream_cutover_policy.handoff_required // false),
    default_dev_plane: ($bootstrap[0].workstream_cutover_policy.default_dev_plane // ""),
    vm_fallback_role: ($bootstrap[0].workstream_cutover_policy.vm_fallback_role // ""),
...
echo "[OK] first real workstream cutover artifacts written to ${OUT}"
