...
for path in \
  ...
  "${OUT}/context-after-manuscript-alignment-labeling.json" \
  "${OUT}/execution-state-after-manuscript-alignment-labeling.json" \
  "${OUT}/execution-receipt-after-manuscript-alignment-labeling.json" \
  "${OUT}/execution-result-links-after-manuscript-alignment-labeling.json" \
  "${OUT}/smoke.json" \
...
jq -e '
  ...
  .manuscript_alignment_label_contract_present == 1 and
  ...
  .prior_b2415_artifacts_present == 1 and
  .prior_b2414_artifacts_present == 1
' "${OUT}/source-summary.json" >/dev/null

jq -e '
  .labels_version == "beagle-manuscript-alignment-label-v1" and
  .rollout_stage == "implementation-and-analysis-canary-live-manuscript-alignment-labeled" and
...
jq -e '
  .status == "ok" and
  .packet.retrieval_context.execution.autonomy_policy_calibration_status == "manuscript-alignment-labeled" and
  .packet.retrieval_context.execution.autonomy_policy_rollout_status == "implementation-and-analysis-canary-live"
' "${OUT}/context-after-manuscript-alignment-labeling.json" >/dev/null

jq -e '
  .state_version == "beagle-plan-execution-state-v1" and
  .autonomy_policy_calibration_status == "manuscript-alignment-labeled" and
  .autonomy_policy_rollout_status == "implementation-and-analysis-canary-live" and
  .current_state == "succeeded"
' "${OUT}/execution-state-after-manuscript-alignment-labeling.json" >/dev/null
...
' "${OUT}/execution-receipt-after-manuscript-alignment-labeling.json" >/dev/null
...
' "${OUT}/execution-result-links-after-manuscript-alignment-labeling.json" >/dev/null
...
echo "[OK] manuscript alignment labeling smoke artifacts validated"
