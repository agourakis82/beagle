...
if [[ "${EXECUTE_STATUS}" != "success" ]]; then
  if [[ "${EXECUTE_ERROR}" == *"401 Unauthorized"* ]] \
    ...
    echo "[BLOCKER] grok_fast live execute reached the provider but failed for external credential/quota reasons: ${EXECUTE_ERROR}" >&2
    exit 2
  fi

  echo "[FAIL] grok_fast execute did not succeed: ${EXECUTE_ERROR:-status=${EXECUTE_STATUS}}" >&2
  exit 1
fi
...
' "${OUT}/execute-grok.json" >/dev/null
...
echo "[OK] grok bridge smoke validation passed"
