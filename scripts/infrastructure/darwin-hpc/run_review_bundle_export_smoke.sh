...
OUT="${OUT:-${ROOT}/.artifacts/darwin-hpc/review-bundle}"
OPERATOR_API_TOKEN="${BEAGLE_OPERATOR_API_TOKEN:-${BEAGLE_API_TOKEN:-}}"
SECRET_NAME="${SECRET_NAME:-beagle-core-secrets}"
STAMP="${STAMP:-$(date +%m%d%H%M%S)}"
WORKSPACE_ID="${WORKSPACE_ID:-b195-review-bundle-${STAMP}}"
...
EXPECTED_MEMORY_DOMAIN="${EXPECTED_MEMORY_DOMAIN:-beagle-review-bundle}"
EXPECTED_MEMORY_TAG="${EXPECTED_MEMORY_TAG:-review-bundle}"
EXPECTED_EXPORT_PROFILE="${EXPECTED_EXPORT_PROFILE:-ro-crate-1.2-bounded}"
...
SEED_RUN_LABEL="${SEED_RUN_LABEL:-b195-review-bundle-${STAMP}-seed}"
...
DOC_FILE="${DOC_FILE:-${ROOT}/docs/darwin/hpc/B195_REVIEW_BUNDLES_AND_ROCRATE_EXPORT.md}"
GO_NO_GO_FILE="${GO_NO_GO_FILE:-${ROOT}/docs/darwin/hpc/B195_GO_NO_GO.md}"
KNOWN_LIMITS_FILE="${KNOWN_LIMITS_FILE:-${ROOT}/docs/darwin/hpc/B195_KNOWN_LIMITS.md}"
CONTRACT_FILE="${CONTRACT_FILE:-${ROOT}/docs/darwin/hpc/contracts/review-bundle-schema.yaml}"
ROCRATE_PROFILE_FILE="${ROCRATE_PROFILE_FILE:-${ROOT}/docs/darwin/hpc/contracts/rocrate-export-profile.yaml}"
DATACITE_FILE="${DATACITE_FILE:-${ROOT}/docs/darwin/hpc/contracts/datacite-citation-block.yaml}"
...
echo "${EXPECTED_EXPORT_PROFILE}" > "${OUT}/expected-export-profile.txt"
...
CONTRACT_PRESENT=0
ROCRATE_PROFILE_PRESENT=0
DATACITE_PRESENT=0

if rg -q "ReviewBundle|build_campaign_review_bundle|ro_crate_metadata" "${REVIEW_BUNDLE_SOURCE_FILE}"; then
  REVIEW_BUNDLE_SOURCE_PRESENT=1
fi
if rg -q "/api/darwin/campaigns/:campaign_id/review-bundle|resolve_campaign_review_bundle|build_campaign_review_bundle" "${HTTP_SOURCE_FILE}"; then
...
[[ -s "${ROCRATE_PROFILE_FILE}" ]] && ROCRATE_PROFILE_PRESENT=1
[[ -s "${DATACITE_FILE}" ]] && DATACITE_PRESENT=1
...
jq -nc \
  ...
  --argjson rocrate_profile_present "${ROCRATE_PROFILE_PRESENT}" \
  --argjson datacite_present "${DATACITE_PRESENT}" \
  '{
    ...
    expected_export_profile: $expected_export_profile,
    ...
    rocrate_profile_present: $rocrate_profile_present,
    datacite_present: $datacite_present
...
MEMORY_TEXT="Review bundle remembers ${EXPECTED_PROGRAM} ${EXPECTED_CAMPAIGN} ${EXPECTED_WORKSTREAM} on ${WORKSPACE_ID} ${SEED_SESSION_ID}."
...
curl_json GET "/api/darwin/campaigns/${EXPECTED_CAMPAIGN}/review-bundle" "${OUT}/review-bundle.json"
jq '.bundle.ro_crate_metadata' "${OUT}/review-bundle.json" > "${OUT}/ro-crate-metadata.json"
stop_port_forward
...
curl_json GET "/api/darwin/campaigns/${EXPECTED_CAMPAIGN}/review-bundle" "${OUT}/review-bundle-after-restart.json"
jq '.bundle.ro_crate_metadata' "${OUT}/review-bundle-after-restart.json" > "${OUT}/ro-crate-metadata-after-restart.json"
stop_port_forward
...
jq -nc \
  ...
  --slurpfile bundle "${OUT}/review-bundle.json" \
  --slurpfile restart "${OUT}/bootstrap-after-restart.json" \
  --slurpfile bundle_after_restart "${OUT}/review-bundle-after-restart.json" \
  '{
    status: (
      if (
        ($bundle[0].bundle.program_id // "") == $expected_program
        and ($bundle[0].bundle.campaign_id // "") == $expected_campaign
        and ($bundle[0].bundle.active_workstream_id // "") == $expected_workstream
        and ($bundle[0].bundle.workspace_id // "") == $workspace_id
        and ($bundle[0].bundle.session_id // "") == $session_id
        and ($bundle[0].bundle.export_profile // "") == $expected_export_profile
        and ($bundle[0].bundle.readiness_state // "") == $expected_readiness_state
        and (($bundle[0].bundle.claims | length) >= 2)
        and (($bundle[0].bundle.payload_manifest | length) >= 4)
        and (($bundle[0].bundle.manuscript_target.id // "") == $expected_manuscript_target)
        and (($bundle[0].bundle.ro_crate_metadata["@graph"] | length) >= 8)
        and (($bundle[0].bundle.citation.related_identifiers | length) >= 3)
        and (($bundle[0].bundle.provenance.activities | length) >= 1)
        and (($restart[0].workspace_id // "") == $workspace_id)
        and (($restart[0].session_id // "") == $session_id)
        and (($restart[0].recovered_session // false) == true)
        and (($bundle_after_restart[0].bundle.program_id // "") == $expected_program)
        and (($bundle_after_restart[0].bundle.campaign_id // "") == $expected_campaign)
        and (($bundle_after_restart[0].bundle.workspace_id // "") == $workspace_id)
        and (($bundle_after_restart[0].bundle.session_id // "") == $session_id)
    ...
    expected_workstream: $expected_workstream,
    expected_experiment: $expected_experiment,
    claim_count: ($bundle[0].bundle.claims | length),
    payload_count: ($bundle[0].bundle.payload_manifest | length),
    readiness_state: ($bundle[0].bundle.readiness_state // null),
...
echo "[OK] review bundle smoke completed"
