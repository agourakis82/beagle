...
OUT="${OUT:-${ROOT}/.artifacts/darwin-hpc/premium-tool-dock-mvp}"
OPERATOR_API_TOKEN="${BEAGLE_OPERATOR_API_TOKEN:-${BEAGLE_API_TOKEN:-}}"
SECRET_NAME="${SECRET_NAME:-beagle-core-secrets}"
STAMP="${STAMP:-$(date +%m%d%H%M%S)}"
WORKSPACE_ID="${WORKSPACE_ID:-b152-tool-dock-${STAMP}}"
...
SEED_RUN_LABEL="${SEED_RUN_LABEL:-b152-tool-dock-${STAMP}-seed}"
...
DOC_FILE="${DOC_FILE:-${ROOT}/docs/darwin/hpc/B152_PREMIUM_TOOL_DOCK_MVP.md}"
GO_NO_GO_FILE="${GO_NO_GO_FILE:-${ROOT}/docs/darwin/hpc/B152_PREMIUM_TOOL_DOCK_GO_NO_GO.md}"
KNOWN_LIMITS_FILE="${KNOWN_LIMITS_FILE:-${ROOT}/docs/darwin/hpc/B152_PREMIUM_TOOL_DOCK_KNOWN_LIMITS.md}"
...
if rg -q "get_workstream_cockpit|get_premium_tool_launch_surface|render_workstream_cockpit_page" "${COCKPIT_SOURCE_FILE}"; then
  COCKPIT_SOURCE_PRESENT=1
fi
if rg -q "/api/darwin/workstreams/:workstream_id/cockpit|/darwin/workstreams/:workstream_id/cockpit|tool-dock/:tool_id" "${HTTP_SOURCE_FILE}"; then
...
curl_html "/darwin/workstreams/${EXPECTED_WORKSTREAM}/cockpit" "${OUT}/cockpit.html"
...
jq -nc \
  ...
  '{
    status: (
      if (
        ($cockpit[0].workstream_id // "") == $expected_workstream
        and ($cockpit[0].envelope.workspace_id // "") == $workspace_id
        and ($cockpit[0].envelope.session_id // "") == $session_id
        and ($cockpit[0].envelope.repo // "") == $expected_repo
        and ($cockpit[0].envelope.branch // "") == $expected_branch
        and ($cockpit[0].envelope.default_dev_plane // "") == $expected_default_dev_plane
        and ($cockpit[0].envelope.vm_fallback_role // "") == $expected_vm_fallback_role
        and ($cockpit[0].envelope.promotion_scope // "") == $expected_promotion_scope
        and ($cockpit[0].envelope.governance_state // "") == $expected_governance_state
        and ($cockpit[0].envelope.governance_last_transition // "") == $expected_last_transition
        and ($cockpit[0].handoff.handoff_present // false) == true
        and ($cockpit[0].last_result.summary.job_id // null) == $published_job_id
        and (($cockpit[0].tool_dock | length) >= 3)
        and ($cursor[0].envelope.session_id // "") == $session_id
        and ($claude[0].envelope.session_id // "") == $session_id
        and ($codex[0].envelope.session_id // "") == $session_id
        and ($restart[0].session_id // "") == $session_id
        and ($restart[0].workspace_id // "") == $workspace_id
        and ($restart[0].recovered_session // false) == true
        and ($cockpit_after_restart[0].envelope.session_id // "") == $session_id
        and ($cockpit_after_restart[0].envelope.workspace_id // "") == $workspace_id
    ...
    expected_governance_state: $expected_governance_state,
    expected_last_transition: $expected_last_transition,
    cockpit_recipe_count: ($cockpit[0].envelope.recipe_count // 0),
    cockpit_handoff_present: ($cockpit[0].handoff.handoff_present // false),
    cockpit_last_result_job_id: ($cockpit[0].last_result.summary.job_id // null),
    cursor_tool_id: ($cursor[0].tool.tool_id // ""),
    claude_tool_id: ($claude[0].tool.tool_id // ""),
    codex_tool_id: ($codex[0].tool.tool_id // ""),
...
echo "[OK] premium tool dock MVP smoke completed"
