...
OUT="${OUT:-/home/devsounio/beagle/.artifacts/darwin-hpc/study-promotion-execution}"
...
for file in \
  ...
  study-promotion-execution.json \
  smoke.json \
...
require_file "${PROOF_OUT}/study-promotion-execution-after-restart.json"
...
jq -e '
  ...
  .source_promotion_execution_present == 1 and
  .doc_present == 1 and
  ...
  .schema_promotion_execution_present == 1 and
...
jq -e '
  .phase == "B26.5" and
  .contract_kind == "study-promotion-execution" and
  .contract_version == "beagle-study-promotion-execution-v1" and
  .same_beagle_owned_identity == true and
  (.promoted_variant_id | length) > 0 and
  (.promoted_run_id | length) > 0 and
  .archived_variant_count >= 1 and
  (.workbench_run_id | length) > 0 and
  (.result_binding_id | length) > 0 and
  (.run_result_identity_receipt_id | length) > 0 and
  (.deterministic_result_binding_id | length) > 0
' "${OUT}/study-promotion-execution.json" >/dev/null

jq -e '
  .phase == "B26.5" and
  .study_id == "'"$(jq -r '.study_id' "${OUT}/study-promotion-execution.json")"'" and
  .workstream_id == "'"$(jq -r '.workstream_id' "${OUT}/study-promotion-execution.json")"'" and
  .workspace_id == "'"$(jq -r '.workspace_id' "${OUT}/study-promotion-execution.json")"'" and
  .session_id == "'"$(jq -r '.session_id' "${OUT}/study-promotion-execution.json")"'" and
  .same_beagle_owned_identity == true and
...
jq -e '
  ...
  .session_id == "'"$(jq -r '.session_id' "${OUT}/study-promotion-execution.json")"'" and
  .same_beagle_owned_identity == true
' "${PROOF_OUT}/study-promotion-execution-after-restart.json" >/dev/null
...
echo "[OK] study promotion execution smoke artifacts validated"
