...
const publicCatalogPath = path.join(rootDir, "public", "project-catalog.json");
...
function buildProjectSovereignSurface(project, { clusterState = null, ide = null } = {}) {
  ...
  const workspaceBootstrapBranch = cleanValue(project?.workspaceBootstrapBranch || "");
...
function buildContextDriftSummary({
  ...
  const normalizedWorkspaceBootstrap = cleanValue(workspaceBootstrapBranch);
  const hasDrift =
  ...
  return {
    ...
    workspaceBootstrapBranch: normalizedWorkspaceBootstrap,
...
async function probeDatasetMetadata(dataset) {
...
function normalizeProjectCatalogProject(project) {
  const sourceBranch = cleanValue(
    project?.branch || project?.preferredPrBase || project?.workspaceBootstrapBranch || "main"
  ) || "main";
  const workspaceBootstrapBranch =
    cleanValue(project?.workspaceBootstrapBranch || project?.branch || "main") || "main";
  ...
  return {
    ...
    workspaceBootstrapBranch,
    preferredPrBase:
      cleanValue(project?.preferredPrBase || sourceBranch || workspaceBootstrapBranch || "main") ||
      "main",
    runtimeCapabilities,
    datasetCatalog,
    viewerDefaults,
...
async function touchSession(projectSlug, clientSessionId, viewer, patch = {}) {
  ...
  const nextSession = {
    ...
    lastDelegationKind: patch.lastDelegationKind || existing.lastDelegationKind || "",
...
async function computeFastWorkspaceMemory(project) {
  ...
  const workspaceBootstrapBranch = cleanValue(
    cachedWorkspace.workspaceBootstrapBranch ||
    rememberedState?.workspace?.workspaceBootstrapBranch ||
    bootstrapProbe?.packet?.branch ||
    project.workspaceBootstrapBranch ||
    ""
  );
  const contextDrift = buildContextDriftSummary({
    contractBranch: cachedWorkspace.branchContract || project.branch,
    contextBranch:
      cachedWorkspace.contextDrift?.contextBranch ||
      bootstrapProbe?.packet?.branch ||
...
async function computeWorkspaceMemory(project) {
  ...
  const workspaceBootstrapBranch = cleanValue(
    env.BEAGLE_WORKSPACE_BRANCH || packet.branch || project.workspaceBootstrapBranch || ""
  );
  const contextDrift = buildContextDriftSummary({
    contractBranch: project.branch,
    contextBranch: packet.branch || env.BEAGLE_WORKSPACE_BRANCH || project.workspaceBootstrapBranch || project.branch,
    liveBranch: git.branch || "",
    workspaceBootstrapBranch,
...
function inferWorkspaceStatefulSetName(project) {
  return (
    ...
    project.workspacePod.replace(/-\d+$/, "")
  );
}
...
// ─── cluster/summary cache ────────────────────────────────────────────
//
// readClusterSummary fans out to 8 parallel kubectl calls — total
// latency is dominated by the slowest, and at burst time it routinely
// blew past the 12s timeout. Cache the result per slug for 15s and
// background-refresh on each access. Stale-while-revalidate: callers
// always get an answer immediately as long as the cache has ANY entry,
// even if it's older than 15s. The truth_mode field on the snapshot
// reflects the staleness so iOS can render accordingly.

const CLUSTER_CACHE_TTL_MS = 15_000;
const CLUSTER_CACHE_HARD_TTL_MS = 5 * 60_000; // drop after 5min stale
const _clusterCache = new Map(); // slug -> { snapshot, fetchedAt, refreshing }

async function refreshClusterSnapshot(slug, project) {
  const slot = _clusterCache.get(slug) || { refreshing: false };
  if (slot.refreshing) return slot.snapshot;
  slot.refreshing = true;
  _clusterCache.set(slug, slot);
  try {
    const snapshot = await withTimeout(
      readClusterSummary(project),
      "cluster summary",
      30_000 // generous; we're off the hot path
    );
    _clusterCache.set(slug, {
      snapshot,
      fetchedAt: Date.now(),
      refreshing: false
    });
...
app.get(
  ...
  jsonResponse(async (req) => {
    ...
    const slot = _clusterCache.get(slug);
    const now = Date.now();
    const age = slot?.fetchedAt ? now - slot.fetchedAt : Infinity;

    // Fast path — fresh cache.
    if (slot?.snapshot && age < CLUSTER_CACHE_TTL_MS) {
      return {
        project,
        cluster: { ...slot.snapshot, _cache: { hit: true, age_ms: age, truth_mode: "observed" } }
      };
    }

    // Stale-while-revalidate — return cached if we have anything within hard TTL,
    // and kick off a background refresh.
    if (slot?.snapshot && age < CLUSTER_CACHE_HARD_TTL_MS) {
      refreshClusterSnapshot(slug, project).catch(() => {});
      return {
        project,
        cluster: {
          ...slot.snapshot,
          _cache: { hit: true, age_ms: age, truth_mode: age > CLUSTER_CACHE_TTL_MS ? "stale" : "observed", refreshing: true }
        }
      };
    }

    // Cold path — block on first read or after long idle.
    const snapshot = await refreshClusterSnapshot(slug, project);
    return {
      project,
      cluster: { ...snapshot, _cache: { hit: false, age_ms: 0, truth_mode: "observed" } }
    };
  })
);
...
// This avoids the path-bound WSS rejecting (with 400) requests destined
